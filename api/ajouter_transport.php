...
// POST { id_sejour, id_transport, nb_passagers, sens }
...
$sens        = in_array($body['sens'] ?? '', ['aller','retour']) ? $body['sens'] : 'aller';
...
try {
    ...
    if (!$transport) { $pdo->rollBack(); json_error('Plus assez de places disponibles.', 409); }

    $prix = round($nbPass * (float)$transport['prix'], 2);

    $pdo->prepare('INSERT INTO sejour_transport (id_sejour, id_transport, nb_passagers, sens, prix) VALUES (?, ?, ?, ?, ?)')->execute([$idSejour, $idTransport, $nbPass, $sens, $prix]);
